function [S, Kc, Kb]=SiStER_Stokes_solver(dx,dy,etas,etan,rho,BC,PARAMS,srhs_xx,srhs_xy)
% [S, Kc, Kb]=SiStER_Stokes_solver(dx,dy,etas,etan,rho,BC,PARAMS,srhs_xx,srhs_xy)
% assemble the Stokes linear system L*S = R, then solves it with a direct
% solver
...
for j=1:Nx
    ...
    %for j=1:Nx
        ...
        if (i==1) || (j==1) || (i==2 && j==2) || (i==2 && j==Nx) || (i==Ny && j==2) || (i==Ny && j==Nx) || (i==IP && j==JP)
            ...
            if (i==IP && j==JP)   % additional pressure cell i=IP, j=JP, P(i,j)=p0cell
        ...
        if ((j==1) && (i<=Ny-1)) || ((j==Nx) && (i<=Ny-1)) || (i==1 && j<=Nx-1 && j>=2) || (i==Ny-1 && j<=Nx-1 && j>=2) || (i==Ny)
            ...
            if ((j==1) && (i<=Ny-1)) %|| ((j==Nx) && (i<=Ny-1))  % left   boundary (why is the right boundary included?
                %              L(invx,invx)=Kb;
            ...
            if i==1 && j<=Nx-1 && j>=2 % upper boundary free slip dvx/dy=0
                ...
                elseif BC.top(1)==0  % no slip
                    ...
                    Lii(nn) = invx;
                    Ljj(nn) = invx+3;
                    Lvv(nn)  = Kc/(dy(i)+dy(i+1));
                    nn = nn+1;
                    
                    Lii(nn) = invx;
                    Ljj(nn) = invx;
                    Lvv(nn)  = -Kc/(1/dy(i)+1/(dy(i)+dy(i+1)));
                    nn = nn+1;
            ...
            if i==Ny-1 && j<=Nx-1 && j>=2 % lower boundary, free slip
                if BC.bot(1)==0 % no slip
                    ...
                    Lii(nn) = invx;
                    Ljj(nn) = invx;
                    Lvv(nn)  = Kc*(1/dy(i)+1/(dy(i-1)+dy(i)));
                    nn = nn+1;
                    
                    Lii(nn) = invx;
                    Ljj(nn) = invx-3;
                    Lvv(nn)  = -Kc/(dy(i-1)+dy(i));
                    nn = nn+1;
        ...
        if (j==1 && i<=Ny-1 && i>=2) || (j==Nx-1 && i<=Ny-1 && i>=2) || (j==Nx) || (i==1 && j<=Nx-1) || (i==Ny && j<=Nx-1)
            ...
            if j==1 && i>=2 && i<=Ny-1  % left boundary without edges no shear dvy/dx=0
            ...
            if j==Nx-1 && i<=Ny-1 && i>=2  % right boundary without edges no shear  dvy/dx=0
